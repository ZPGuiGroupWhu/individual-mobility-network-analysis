...
from .scale_fitter_no_grid import *
...
def _worker_bootstrap_pval(inputs):
    ...
    L = total_likelihood(source_target[random_indices], cell_attractiveness, fitted_cell_p_change)
    np.random.seed(seed+1)
    random_indices = np.random.randint(0, N, 300)
    L_prev = total_likelihood(source_target_prev[random_indices], cell_attractiveness_prev, fitted_cell_p_change_prev)
    return L, L_prev
...
def bootstrap_pval(series_prev, series, stat_test, num_iter=1000, nprocs = 10):
    ...
    fitted_cell_p_change,_ = find_cell_p_change(source_target, n_scales, series)
    
    # Cell attractiveness and cell p change of prev
    cell_attractiveness_prev = {}
    compute_cell_attractiveness(series_prev, cell_attractiveness_prev, 0)
    fitted_cell_p_change_prev,_ = find_cell_p_change(source_target_prev, n_scales_prev, series_prev)
...
def general_network_pdist(
    G,
    distance_function,
    coord_attr='loc'
):
    """
    Calculate pairwise shortest-path distances between all node pairs in a network,
    where edge weights are computed on-the-fly from node spatial coordinates.

    Parameters
    ----------
    G : networkx.Graph or DiGraph
        Network without pre-defined edge weights
    distance_function : callable
        Function computing spatial distance between two points
        distance_function(p1, p2) -> float
    coord_attr : str, default='pos'
        Node attribute storing spatial coordinates

    Returns
    -------
    result : np.ndarray, shape = (N*(N-1)//2,)
        Condensed distance vector (same ordering as scipy.spatial.distance.pdist)
    """

    nodes = list(G.nodes())
    n = len(nodes)

    # ---- Step 1: create a weighted view of G (no modification to original G) ----
    Gw = nx.Graph() if not G.is_directed() else nx.DiGraph()
    Gw.add_nodes_from(nodes)

    for u, v in G.edges():
        p_u = np.asarray(G.nodes[u][coord_attr])
        p_v = np.asarray(G.nodes[v][coord_attr])

        w = distance_function(p_u, p_v)
        Gw.add_edge(u, v, spatial_dist=w)

    # ---- Step 2: all-pairs shortest path ----
    all_lengths = dict(nx.all_pairs_dijkstra_path_length(Gw, weight='spatial_dist'))

    # ---- Step 3: condensed distance vector ----
    result = np.zeros(n * (n - 1) // 2, dtype=np.float64)
    vec_idx = 0
    ...
    for i in range(n - 1):
        ...
        for j in range(i + 1, n):
            v = nodes[j]
            result[vec_idx] = lengths_u.get(v, np.inf)
            vec_idx += 1
...
def convex_hull(points, to_return='points'):
    ...
    if to_return == "geoarea":
        ...
        return area({'type':'Polygon','coordinates':[points[hull.vyertices].tolist()]})
...
def get_container_labels(series):
    ...
    Example
    -------
        >>> container_hierarchy = get_container_labels(series)
        >>> container_hierarchy
        {   # scale
            2: {
                # container: labels
    ...
    container_hierarchy = defaultdict(set)

    for cell in containers:
        for s in range(scale-1):
            container_hierarchy[tuple(cell[:s+1])].add(cell[-1])
    '''
    for cell in containers:
        for s in range(1, scale):
            container_key = tuple(cell[:s])
            container_hierarchy[s][container_key].append(stop_label)
    '''
