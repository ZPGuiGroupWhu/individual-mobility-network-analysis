...
def find_cell_p_change(source_target, n_scales, series):
    ...
    d_last = n_scales - dictionary[last_location]
    if d_last in cell_p_change_dict_by_d:
        cell_p_change_dict[last_location] = cell_p_change_dict_by_d[d_last]
    else:
        print("Warning: scale fitting encountered an exception")
        avg_p = np.mean(
            np.array(list(cell_p_change_dict_by_d.values())),
            axis=0
        )
        cell_p_change_dict[last_location] = avg_p
